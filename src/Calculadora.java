...
public class Calculadora {
    ...
    public static void main(String[] args) {
        ...
        botaoCalcular.addActionListener((ActionEvent e) -> {
            try {
                double preco    = parseDoubleCampo(campoPreco, "Preço de venda");
                double custo    = parseDoubleCampo(campoCusto, "Custo do produto");
                double pctAfi   = parseDoubleCampo(campoAfiliado, "Comissão do afiliado") / 100;

                if (preco <= 0 || custo < 0 || pctAfi < 0) {
    ...
    static double parseDoubleCampo(JTextField campo, String nomeCampo) {
        String texto = campo.getText().trim();
        if (texto.isEmpty()) {
            throw new NumberFormatException(nomeCampo + " não informado");
        }
